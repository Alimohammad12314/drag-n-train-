...
app = FastAPI(title="Drag n' Train AI API", version="8.2.1") 
    ...
    if type == "shap_summary":
        ...
        def predict_fn_for_lime(numpy_array):
            ...
            for col_name in categorical_features_names:
                ...
                rounded_codes = np.round(df[col_name])
                
                
                mapped_strings = rounded_codes.map(inv_map)
                
                
                mode_value = X_train[col_name].mode()[0]
                
               
                df[col_name] = mapped_strings.fillna(mode_value)
        ...
        for col_name in categorical_features_names:
            ...
            instance_to_explain_numeric[col_name] = mappings[col_name].get(instance_to_explain_numeric[col_name], 0)

       
        explanation = explainer.explain_instance(
            instance_to_explain_numeric.to_numpy(), predict_fn_for_lime, num_features=10
        )
